...
function activate(context) {
    ...
    context.subscriptions.push(vscode_1.default.languages.registerDocumentFormattingEditProvider("blade", {
        provideDocumentFormattingEdits(document, vscodeOpts) {
            ...
            if ((runtimeConfig === null || runtimeConfig === void 0 ? void 0 : runtimeConfig.sortTailwindcssClasses) || extConfig.sortTailwindcssClasses) {
                ...
                try {
                    ...
                    tailwindConfig.tailwindcssConfigPath = require.resolve('tailwindcss/lib/public/default-config');
                }
            }
            const options = Object.assign(Object.assign({ vsctm: vsctmModule, oniguruma: onigurumaModule, indentSize: extConfig.indentSize, wrapLineLength: extConfig.wrapLineLength, wrapAttributes: extConfig.wrapAttributes, useTabs: extConfig.useTabs, sortTailwindcssClasses: extConfig.sortTailwindcssClasses, sortHtmlAttributes: (_b = extConfig.sortHtmlAttributes) !== null && _b !== void 0 ? _b : 'none', noMultipleEmptyLines: extConfig.noMultipleEmptyLines }, runtimeConfig), tailwindConfig);
            const progressMessage = isLargeFile(document) ? messages_1.messages.largeFileFormattingMessage : messages_1.messages.formattingMessage;
...
function showWelcomeMessage(context) {
    ...
    const previousVersionArray = previousVersion ? previousVersion.split('.').map((s) => Number(s)) : [0, 0, 0];
    const currentVersionArray = currentVersion.split('.').map((s) => Number(s));
    ...
    if (message) {
        vscode_1.default.window.showInformationMessage(message, '⭐️ Star on Github', '🐞 Report Bug')
            .then(function (val) {
            if (val === '🐞 Report Bug') {
...
class Logger {
    constructor() {
        this.outputChannel = vscode_1.window.createOutputChannel("BladeFormatter");
        this.logLevel = "INFO";
    }
