    ...
    provideCodeActions(document, range, context, token) {
        ...
        for (const diagnostic of diagnostics) {
            if (diagnostic.range.contains(range) &&
                ...
                const action = new vscode.CodeAction(`Update "${diagnostic.property}" with Data Dictionary value`, vscode.CodeActionKind.QuickFix);
                action.command = {
                ...
                actions.push(action);
                const action2 = new vscode.CodeAction(`Update "${diagnostic.path.slice(-1)}" with all Data Dictionary properties`, vscode.CodeActionKind.QuickFix);
                action2.command = {
