...
`),(n=s.errors[0].params)!=null&&n.allowedValues&&console.log((r=s.errors[0].params)==null?void 0:r.allowedValues),g.default.exit(1)}}}async processPaths(){await Promise.all(w.default.map(this.paths,async e=>this.processPath(e)))}async processPath(e){await X.globFiles(e).then(t=>w.default.map(t,n=>I.default.relative(".",n))).then(t=>this.filterFiles(t)).then(this.fulFillFiles).then(t=>this.formatFiles(t))}static globFiles(e){return new Promise((t,n)=>{(0,We.default)(e,(r,s)=>r?n(r):t(s))})}async filterFiles(e){if(this.ignoreFile==="")return e;let t=/^\.\.*/,n=w.default.filter(e,a=>t.test(I.default.relative(".",a))),r=w.default.xor(e,n),s=(0,ze.default)().add(this.ignoreFile).filter(r);return w.default.concat(n,s)}static fulFillFiles(e){return this.targetFiles.push(e),Promise.resolve(e)}async formatFiles(e){await Promise.all(w.default.map(e,async t=>this.formatFile(t)))}async formatFile(e){await this.findTailwindConfig(e),await this.readRuntimeConfig(e),await Z(e).then(t=>Promise.resolve(t.toString("utf-8"))).then(t=>new k(this.options).formatContent(t)).then(t=>this.checkFormatted(e,t)).then(t=>this.writeToFile(e,t)).catch(t=>{this.handleError(e,t)})}async checkFormatted(e,t){this.printFormattedOutput(e,t);let n=H.default.readFileSync(e,"utf-8"),r=N(n),s=N(t),a=ve(e,r,s);return this.diffs.push(a),this.outputs.push(t),a.length>0&&((this.options.progress||this.options.write)&&g.default.stdout.write($.default.green("F")),this.options.checkFormatted&&(g.default.stdout.write(`${e}
`),g.default.exitCode=1),this.formattedFiles.push(e)),a.length===0&&(this.options.progress||this.options.write)&&g.default.stdout.write($.default.green(".")),Promise.resolve(t)}printFormattedOutput(e,t){this.options.write||this.options.checkFormatted||(g.default.stdout.write(`${t}`),w.default.last(this.paths)===e||w.default.last(this.targetFiles)===e)||(this.paths.length>1||this.targetFiles.length>1)&&g.default.stdout.write(`
`)}writeToFile(e,t){!this.options.write||this.options.checkFormatted||t.length===0||w.default.isNull(t)||w.default.isEmpty(t)||H.default.writeFile(e,t,n=>{n&&(g.default.stdout.write(`${$.default.red(n.message)}
...
Formatted Files: 
`)),w.default.each(this.formattedFiles,e=>g.default.stdout.write(`${$.default.bold(e)}
`))}printDifferences(){if(!!this.options.diff){if(g.default.stdout.write($.default.bold(`
Differences: 

`)),w.default.filter(this.diffs,e=>e.length>0).length===0){g.default.stdout.write((0,$.default)(`No changes found. 

`));return}w.default.each(this.diffs,e=>Pe(e))}}printErrors(){w.default.isEmpty(this.errors)||(g.default.stdout.write($.default.red.bold(`
Errors: 

`)),w.default.each(this.errors,e=>g.default.stdout.write(`${oe.default.format(e)}
