...
const preserve_1 = require("@xliic/preserving-json-yaml-parser/lib/preserve");
...
class JsonNodeValue {
    ...
    getParent(root) {
        if (this.pointer !== "") {
            ...
            return new JsonNodeValue((0, preserving_json_yaml_parser_1.find)(root, parentPointer), parentPointer);
        }
        return undefined;
    }
    getRange(root) {
        ...
        if (parent) {
            for (const key of Object.keys(parent.value)) {
                if (key === myKey && this.value === parent.value[key]) {
                    ...
                    const loc = (0, preserve_1.getPreservedLocation)(container, key);
                    if (loc) {
...
function findJsonNodeValue(root, pointer) {
    const value = (0, preserving_json_yaml_parser_1.find)(root, pointer);
...
function comparator(container) {
    return function (key1, key2) {
        return (getOffset((0, preserve_1.getPreservedLocation)(container, key1)) -
            getOffset((0, preserve_1.getPreservedLocation)(container, key2)));
    };
}
