...
function displayData(csvParseResult, csvReadConfig) {
    ...
    if (hot) {
        if (previousSelectedCell === null || previousViewportOffsets === null) {
            ...
            if (csvParseResult) {
                cellToSelect = calcHotCellToSelectFromCurosPos(initialVars.sourceFileCursorLineIndex, initialVars.sourceFileCursorColumnIndex, initialVars.isCursorPosAfterLastColumn, csvParseResult, csvReadConfig);
            }
            else {
                cellToSelect = {
                    rowIndex: 0,
