...
`);return i.default.chain(a).map((l,h)=>h===0?l.trim():s+l).value().join(`
`)}restoreBladeDirectivesInScripts(e){let t=new RegExp(`^(.*?)${this.getBladeDirectivePlaceholder("(\\d+)")}`,"gm"),n=i.default.replace(e,t,(r,s,a)=>this.indentBladeDirectiveBlock(s,this.bladeDirectives[a]));return t.test(e)&&(n=this.restoreBladeDirectivesInScripts(n)),n}async formatPreservedBladeDirectives(e){return $e.default.map(e,async t=>{let n=await this.formatAsHtml(t);return(await this.formatAsBlade(n)).trimRight(`
`)})}restoreBladeComment(e){return new Promise(t=>t(e)).then(t=>i.default.replace(t,new RegExp(`${this.getBladeCommentPlaceholder("(\\d+)")}`,"gms"),(n,r)=>`{{-- ${this.bladeComments[r].trim()} --}}`))}restoreBladeBrace(e){return new Promise(t=>t(e)).then(t=>i.default.replace(t,new RegExp(`(.*?)${this.getBladeBracePlaceholder("(\\d+)")}`,"gm"),(n,r,s)=>{let a=this.bladeBraces[s];return a.trim()===""?`${r}{{${a}}}`:this.isInline(a)?`${r}{{ ${P(a,1e3,!1).replace(/([\n\s]*)->([\n\s]*)/gs,"->").split(`
...
`),((n=s.errors[0].params)==null?void 0:n.allowedValues)&&console.log((r=s.errors[0].params)==null?void 0:r.allowedValues),u.default.exit(1)}}async processPaths(){await Promise.all(f.default.map(this.paths,async e=>this.processPath(e)))}async processPath(e){await R.globFiles(e).then(t=>f.default.map(t,n=>k.default.relative(".",n))).then(t=>this.filterFiles(t)).then(this.fulFillFiles).then(t=>this.formatFiles(t))}static globFiles(e){return new Promise((t,n)=>{(0,Le.default)(e,(r,s)=>r?n(r):t(s))})}async filterFiles(e){if(this.ignoreFile==="")return e;let t=/^\.\.*/,n=f.default.filter(e,a=>t.test(k.default.relative(".",a))),r=f.default.xor(e,n),s=(0,Re.default)().add(this.ignoreFile).filter(r);return f.default.concat(n,s)}static fulFillFiles(e){return this.targetFiles.push(e),Promise.resolve(e)}async formatFiles(e){await Promise.all(f.default.map(e,async t=>this.formatFile(t)))}async formatFile(e){await this.readRuntimeConfig(e),await H(e).then(t=>Promise.resolve(t.toString("utf-8"))).then(t=>new F(this.options).formatContent(t)).then(t=>this.checkFormatted(e,t)).then(t=>this.writeToFile(e,t)).catch(t=>{this.handleError(e,t)})}async checkFormatted(e,t){this.printFormattedOutput(e,t);let n=I.default.readFileSync(e,"utf-8"),r=E(n),s=E(t),a=ce(e,r,s);return this.diffs.push(a),this.outputs.push(t),a.length>0&&((this.options.progress||this.options.write)&&u.default.stdout.write(w.default.green("F")),this.options.checkFormatted&&(u.default.stdout.write(`${e}
...
`));return}f.default.each(this.diffs,e=>he(e))}}printErrors(){f.default.isEmpty(this.errors)||(u.default.stdout.write(w.default.red.bold(`
Errors: 

`)),f.default.each(this.errors,e=>u.default.stdout.write(`${Y.default.format(e)}
`)))}};async function je(){let o=await(0,Fe.default)((0,De.hideBin)(process.argv)).usage("Usage: $0 [options] [file glob | ...]").example('$0 "resources/views/**/*.blade.php" --write',"Format all files in views directory").option("check-formatted",{alias:"c",type:"boolean",description:"Only checks files are formatted or not",default:!1}).option("write",{alias:"w",type:"boolean",description:"Write to file",default:!1}).option("diff",{alias:"d",type:"boolean",description:"Show diffs",default:!1}).option("end-with-newline",{alias:"e",type:"boolean",description:"End output with newline",default:!0}).option("indent-size",{alias:"i",type:"integer",description:"Indentation size",default:4}).option("wrap-line-length",{alias:"wrap",type:"integer",description:"The length of line wrap size",default:120}).option("wrap-attributes",{alias:"wrap-atts",type:"string",description:"The way to wrap attributes",default:"auto"}).option("sort-tailwindcss-classes",{alias:"sort-classes",type:"boolean",description:"Sort tailwindcss classes",default:!1}).option("progress",{alias:"p",type:"boolean",description:"Print progress",default:!1}).option("stdin",{type:"boolean",description:"format code provided on <STDIN>",default:!1}).option("config",{alias:["runtimeConfigPath"],type:"string",description:"Use this configuration, overriding .bladeformatterrc config options if present",default:null}).option("ignore-path",{alias:["ignoreFilePath"],type:"string",description:"Specify path of ignore file",default:null}).help("h").alias("h","help").epilog("Copyright Shuhei Hayashibara 2019"),e=await Ae.promises.readFile(require.resolve("vscode-oniguruma/release/onig.wasm"));if(await(0,Te.loadWASM)(e.buffer),o.argv.stdin){await process.stdin.pipe((0,ke.default)({encoding:"string"},n=>new R(o.argv).format(n).then(r=>{r!==void 0&&process.stdout.write(r)}).catch(r=>{process.stdout.write(`${r.toString()}
`),process.exit(1)})));return}if(o.argv._.length===0){o.showHelp();return}await new R(o.argv,o.argv._).formatFromCLI()}module.exports=He(Qe);0&&(module.exports={});
